...
async def _seed_static_knowledge_graph(wiki: Any, graph_mgr: MemoryGraphManager) -> None:
    ...
    nodes = [
        ...
        HeterogeneousGraphNode(
            ...
            description="Tool API for opening a URL or first search result in Google Chrome from an execution plan.",
            labels=["api", "chrome", "url", "tool"],
            source_type="api_doc",
        ...
        HeterogeneousGraphNode(
            ...
            description="Tool API for a bounded browser observe-decide-act workflow. The current runtime launches the query and records observations/actions; full arbitrary click execution requires a DOM or screenshot controller.",
        ...
        HeterogeneousGraphNode(
            ...
            description="Runtime-side observer for browser workflows. It should provide screenshots, DOM snapshots, visible text, candidate clickable elements, and action evidence to the execution agent.",
            labels=["tool", "browser", "gui", "observation", "runtime"],
            source_type="tool_doc",
            metadata={"planned_observation_channels": ["screenshot", "dom_snapshot", "visible_text", "action_result"]},
        ),
        HeterogeneousGraphNode(
        ...
        HeterogeneousGraphNode(
            ...
            node_type=GraphNodeType.SCRIPT,
            name="Google first-result URL command",
            description="Static script source for opening Google's first result for a query via btnI.",
            labels=["script", "search", "first-result", "chrome", "url"],
            source_type="script",
            metadata={"url_template": "https://www.google.com/search?q=<query>&btnI=I"},
        ),
        HeterogeneousGraphNode(
            ...
            metadata={
                "pseudocode": [
                    ...
                    "execute click/type/navigate action and store evidence",
                ],
...
async def _seed_demo_skills(wiki: MemoryWikiManager) -> None:
    ...
    demos = [
        ...
        dict(
            ...
            description="Search the web for a user-provided query and open the first search result in Chrome.",
            skill_type=SkillType.FUNCTIONAL,
            domain="web",
            granularity_level=2,
            tags=["host", "browser", "chrome", "search", "first-result", "website", "generic"],
            interface=iface(
                ...
                pre=["The task asks to open the first search result for a query"],
                post=["Chrome is launched with a first-result search URL"],
            ),
            implementation=SkillImplementation(
                language="python",
            ...
            test_cases=[
                testcase(
                    "open first result for a query",
                    {"query": "国科大夏令营"},
                    {"launched": True},
                    tags=["validation", "search", "first-result"],
                )
            ],
        ),
        dict(
