...
# 在 postgres_db.py 末尾添加

class ExecutionHistoryRepository:
    """执行历史持久化仓储 — 基于 SkillExecutionRecordORM。"""

    def __init__(self, conn: PostgresConnection) -> None:
        self._conn = conn

    async def save_plan_history(self, history_item: Dict[str, Any]) -> None:
        """保存计划级执行历史（作为特殊的 execution record）。"""
        created_at_str = history_item.get("created_at")
        created_dt = datetime.fromisoformat(created_at_str) if created_at_str else None

        async with self._conn.session() as session:
            orm = SkillExecutionRecordORM(
                record_id=str(uuid.uuid4()),
                skill_id="__plan__",
                skill_version="n/a",
                task_id=history_item["execution_id"],
                input_data_json=json.dumps({"goal": history_item["goal"]}),
                output_data_json=json.dumps({
                    "status": history_item["status"],
                    "step_count": history_item["step_count"],
                    "success_count": history_item["success_count"],
                    "retrieved_skill_count": history_item["retrieved_skill_count"],
                }),
                status=history_item["status"],
                latency_ms=history_item["total_latency_ms"],
    ...
    async def list_history(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        async with self._conn.session() as session:
            result = await session.execute(
                select(SkillExecutionRecordORM)
                .where(SkillExecutionRecordORM.skill_id == "__plan__")
                .order_by(desc(SkillExecutionRecordORM.created_at))
                .limit(limit)
            )
            rows = result.scalars().all()
            history = []
            for row in rows:
                output = json.loads(row.output_data_json or "{}")
                input_ = json.loads(row.input_data_json or "{}")
                history.append({
                    ...
                    "retrieved_skill_count": output.get("retrieved_skill_count", 0),
        ...
        async with self._conn.session() as session:
            result = await session.execute(
                select(func.count()).where(
                    SkillExecutionRecordORM.skill_id == "__plan__"
                )
            )
            return result.scalar() or 0