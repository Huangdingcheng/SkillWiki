...
You are the SkillOS Ctx2Skill-lite agent.
...
class Ctx2SkillLiteExtractor:
    ...
    def process_past_skills(
        ...
        *,
        ...
        """Normalize legacy or external Skill descriptions into SkillOS candidates."""
        import uuid
    ...
    def _llm_payload(self, raw_content: str, source_type: str, actions: List[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            resp = self._llm.chat(
                [
                    Message.system("You are the SkillOS Ctx2Skill-lite agent. Return JSON only."),
                    Message.user(prompt),
                ]
            )
            self.last_token_usage = resp.total_tokens
...
def _ctx2skill_payload_from_candidate(
    ...
    *,
    ...
    challenge = {
        ...
        "task": f"Convert legacy Skill {candidate['name']} into canonical SkillOS representation.",
        "rubric": [
        ...
        "expected_evidence": "Canonical SkillOS schema fields plus graph relation preview.",
...
def _past_skill_output_schema(entry: Dict[str, Any], name: str, description: str) -> Dict[str, Any]:
    ...
    properties: Dict[str, Any] = {
        "result": {
            "type": "object",
            "description": "Structured task result produced by the normalized SkillOS Skill.",
        },
        "evidence": {
            "type": "array",
            "items": {"type": "string"},
...
def _past_skill_postconditions(entry: Dict[str, Any], name: str, description: str) -> List[str]:
    text = _entry_text(entry, name, description).lower()
    postconditions = [
        "output.result preserves the original legacy Skill capability in SkillOS form.",
...
def _past_skill_prompt_template(
    ...
    return (
        f"You are executing the imported legacy Skill '{name}' as a SkillOS {skill_type} Skill.\n"
...
def _prompt_safe_excerpt(text: str, *, max_chars: int) -> str:
    ...
    if len(text) <= max_chars:
        return text
    return text[:max_chars].rstrip() + "\n[excerpt truncated for SkillOS prompt safety]"
