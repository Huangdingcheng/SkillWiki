...
$StartScript = Join-Path $ScriptDir "Start-SkillOSDemo.ps1"
...
if (-not (Test-HttpOk $backendHealth) -or -not (Test-HttpOk $frontendRoot)) {
    Write-Host "SkillOS demo service is not fully ready. Starting backend and frontend first..."
    if (-not (Test-Path $StartScript)) {
        throw "Start script not found: $StartScript"
    }
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $StartScript -NoOpen
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to start SkillOS demo service."
    }
    $state = Get-LauncherState
...
Write-Host ""
Write-Host "Restoring SkillOS demo state..."
...
& $PythonExe @argsList
if ($LASTEXITCODE -ne 0) {
    throw "SkillOS demo-state restore failed with exit code $LASTEXITCODE"
}
